                      ...
                      <div class="row no-no-gutters ">
                        ...
                        <div class="col-md-6">
                          <div class="form-group">
                            ...
                            <select class="form-control" v-model="form.food_categories_id"  :class="{ 'is-invalid': form.errors.has('food_categories_id') }">
                                <option v-for="item in categories" :key="item.id" :value="item.id">{{item.title}}</option>
                            </select>
                            <span v-if="errors.food_categories_id" class="error">{{ errors.food_categories_id[0] }}</span>
                          </div>
                          <div class="form-group">
                            <label>Title</label>
...
export default {
  ...
  methods: {
    ...
    creating() {
      ...
      formData.append("food_categories_id", this.form.food_categories_id);
      formData.append("_method", "put");
      let id = this.$route.params.id;
      axios
        .post("api/food/" + id, formData, {
          headers: {
        ...
        .then((res) => {
          ...
          this.$router.push({ path: "/admin/food-staff/" });
          Toast.fire({
            icon: "success",
    ...
    loadItem() {
      let id = this.$route.params.id;
      axios.get("api/food/" + id).then((response) => {
