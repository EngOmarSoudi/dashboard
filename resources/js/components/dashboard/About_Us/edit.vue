                    ...
                    <form>
                      <div class="row no-no-gutters ">
                        ...
                        <div class="col-md-12 mt-3 text-right">
                          ...
                          <div class="form-group ">
                            <label>العنوان</label>
                            <input v-model="form.ar_title" type="text" class="form-control text-right" :class="{ 'is-invalid': form.errors.has('ar_title') }" placeholder="ادخل العنوان" name="ar_title" />

                            <span v-if="errors.ar_title" class="error">{{ errors.ar_title[0] }}</span>

                          </div>
                          <div class="form-group">
                            <label> المحتوى</label>
                            <textarea v-model="form.ar_content" class="form-control text-right" :class="{ 'is-invalid': form.errors.has('ar_content') }" rows="7" placeholder="ادخل المحتوى" name="ar_content"></textarea>
                            <span v-if="errors.ar_content" class="error">{{ errors.ar_content[0] }}</span>
                          </div>
                        </div>
                      </div>
                    </form>
...
export default {
  ...
  methods: {
    getItemImage() {
      return "images/about_us/" + this.form.image;
    },
    upload_image(e) {
    ...
    creating() {
      ...
      formData.append("ar_title", this.form.ar_title);
      formData.append("ar_content", this.form.ar_content);
      formData.append("image", this.form.image);
      formData.append("_method", "put");
      let id = this.$route.params.id;
      axios
        .post("api/aboutUs/" + id, formData, {
          headers: {
        ...
        .then((res) => {
          ...
          this.$router.push({ path: "/admin/about-us/" });
          Toast.fire({
            icon: "success",
    ...
    loadItem() {
      let id = this.$route.params.id;
      axios.get("api/aboutUs/" + id).then((response) => {
