                    ...
                    <div class="row no-no-gutters ">
                      ...
                      <div class="col-md-6">
                        <div class="form-group">
                          <label>Name</label>
                          <input v-model="form.name" type="text" class="form-control" :class="{ 'is-invalid': form.errors.has('name') }" placeholder="Enter Name" name="name" />
                          <span v-if="errors.name" class="error">{{ errors.name[0] }}</span>
                        </div>
                         <div class="form-group text-right">
                          <label>الاسم باللغة العربية</label>
                          <input v-model="form.name_ar" type="text" class="form-control text-right" :class="{ 'is-invalid': form.errors.has('name_ar') }" placeholder="الاسم باللغة العربية" name="name_ar" />
                          <span v-if="errors.name_ar" class="error">{{ errors.name_ar[0] }}</span>
                        </div>
                        <div class="form-group">
                          <label>Website</label>
                          <input v-model="form.url" type="text" class="form-control" :class="{ 'is-invalid': form.errors.has('url') }" placeholder="Enter URL" name="url" />
                          <span v-if="errors.url" class="error">{{ errors.url[0] }}</span>
                        </div>
                      </div>
                    </div>
                  </form>
...
export default {
  ...
  methods: {
    ...
    loadItem() {
      let id = this.$route.params.id;
      axios.get("api/agencies/" + id).then((response) => {
