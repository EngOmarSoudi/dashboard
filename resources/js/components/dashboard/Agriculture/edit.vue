                      ...
                      <div class="row no-no-gutters ">
                        <div class="col-md-6" style="height: 300px;">
                          <label class="placeholder" id="preview" :style="{
                            ...
                            <input name="image" type="file" class="file-input" :class="{ 'is-invalid': form.errors.has('image') }" @change="upload_image($event)" />
                        ...
                        <div class="col-md-6">
                          <div class="form-group">
                            ...
                            <select class="form-control" v-model="form.agriculture_categories_id"  :class="{ 'is-invalid': form.errors.has('agriculture_categories_id') }">
                                <option v-for="item in categories" :key="item.id" :value="item.id">{{item.title}}</option>
                            </select>
                            <span v-if="errors.agriculture_categories_id" class="error">{{ errors.agriculture_categories_id[0] }}</span>
                          </div>
                          <div class="form-group">
                            <label>Title</label>
                          ...
                          <div class="form-group">
                            <label>Content</label>
                            <textarea v-model="form.content" class="form-control" :class="{ 'is-invalid': form.errors.has('content') }" rows="7" placeholder="Enter content" name="content"></textarea>
                        ...
                        <div class="col-md-12 mt-3 text-right">
                          ...
                          <div class="form-group">
                            <label> المحتوى</label>
                            <textarea v-model="form.content_ar" class="form-control text-right" :class="{ 'is-invalid': form.errors.has('content_ar') }" rows="7" placeholder="ادخل المحتوى" name="content_ar"></textarea>
  ...
  methods: {
    ...
    upload_image(e) {
     ...
     displayCategoriesItems() {
            axios.get("api/agriculture_categories").then(response => {
    ...
    creating() {
      ...
      formData.append("content_ar", this.form.content_ar);
      formData.append("image", this.form.image);
      formData.append("agriculture_categories_id", this.form.agriculture_categories_id);
      formData.append("_method", "put");
      let id = this.$route.params.id;
      axios
        .post("api/agriculture/" + id, formData, {
          headers: {
        ...
        .then((res) => {
          ...
          this.$router.push({ path: "/admin/agriculture/" });
          Toast.fire({
            icon: "success",
    ...
    loadItem() {
      let id = this.$route.params.id;
      axios.get("api/agriculture/" + id).then((response) => {
