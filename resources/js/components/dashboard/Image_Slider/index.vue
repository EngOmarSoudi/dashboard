              ...
              <div class="head-fixed">
                <div class="min-div px-4 py-3">
                  ...
                  <div class="min-div_list_icon">
                    ...
                    <router-link to="/admin/image-slider/create">
                      <img :src="images.icon_add" class="btn " v-b-popover.hover.top="'Add an Image'" />
                    </router-link>
              ...
              <div class="min-div px-4 py-3 my-5">
                ...
                <div class="min-div_list_item mt-4">
                  <div class=" row no-gutters my-4  position-relative" v-for="(item, i) in filteredList" :key="i">
                    <img class="min-div_list_item_img col-md-3" :src="'images/imageslider/'+getFileName(item.photo)" />
<!--                     item.photo.filename  -->
                    <div class="card-body col-md-9 py-2 px-4">
                      ...
                      <div class="d-flex justify-content-end" style="position: absolute; bottom: 8px; right: 0 ">
                        <img :src="images.show_img" width="22px" v-show="item.GI == 1" class="btn-card_show mx-3" @click="isNotdisplay(item.id)" />
                        <img :src="images.hide_img" width="22px" v-show="item.GI == 0" class="btn-card_show mx-3" @click="isdisplay(item.id)" />

                        <div class="btn-card_edit mx-3">
                          <router-link :to="'/admin/image-slider/edit/' + item.id">
...
export default {
  ...
  methods: {
    displayItems() {
      ...
      axios.get("api/imageslide").then((response) => {
    ...
    isdisplay(id) {
      this.isDisabled = true;
      setTimeout(() => this.displayItems(), 2000);
      axios.put("api/imageSlideIsDisplay/" + id).then((res) => {
        Toast.fire({
          icon: "success",
          title: "Item is displayed",
            timer: 2000,
    ...
    isNotdisplay(id) {
      ...
      axios.put("api/imageSlideIsDisplay/" + id).then((res) => {
        Toast.fire({
          icon: "info",
          title: "Item is not be displayed",
            timer: 2000,
            timerProgressBar: true,
  ...
  created() {
   ...
   computed: {
    filteredList() {
    if(this.search){
      return this.imageSlider.filter((item)=>{
